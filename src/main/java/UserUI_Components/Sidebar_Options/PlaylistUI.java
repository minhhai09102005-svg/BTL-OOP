...
public class PlaylistUI extends StackPane {                           // [OLD] root node là StackPane
    ...
    public static void add(Song s) {                                  // [OLD] thêm bài nếu chưa có
        if (s != null && !DATA.contains(s)) {
            DATA.add(s);                                              // [OLD] tránh null/duplicate
            s.setPlaylist(true);
            
            // ⭐ Lưu vào database
            try {
                Backend.Controller.SessionController session = Backend.Controller.SessionController.getInstance();
                if (session.isLoggedIn() && s.getFilePath() != null) {
                    Backend.Database.SongDAO songDAO = new Backend.Database.SongDAO();
                    Backend.Model.Song dbSong = songDAO.getSongByFilePath(s.getFilePath());
                    ...
                    if (dbSong == null) {
                        ...
                        newSong.setSongTitle(s.getName());
                        newSong.setArtists(s.getArtist());
                        newSong.setDuration(s.getDurationSeconds());
                        newSong.setFilePath(s.getFilePath());
                    ...
                    if (dbSong != null) {
                        ...
                        boolean success = session.getSongService().addToPlaylist(userId, songId);
                        if (success) {
        ...
        try {
            ...
            if (session.isLoggedIn() && s.getFilePath() != null) {
                ...
                if (dbSong != null) {
                    int userId = session.getCurrentUserId();
                    int songId = dbSong.getId();
                    boolean success = session.getSongService().removeFromPlaylist(userId, songId);
                    if (success) {
                        System.out.println("✅ Đã xóa khỏi Playlist (DB)");
                    }
                }
            }
        } catch (Exception ex) {
            System.err.println("⚠️ Lỗi xóa playlist khỏi database: " + ex.getMessage());
        }
    }
    public static ObservableList<Song> items() { return DATA; }       // [OLD] expose list (read-only view bên ngoài)
    
    /**
     * Load playlist từ database
     */
    public static void loadFromDatabase() {
        try {
            ...
            List<Backend.Model.Song> dbPlaylist = songService.getPlaylistSongs(userId);
            
            Platform.runLater(() -> {
                DATA.clear();
                
                for (Backend.Model.Song dbSong : dbPlaylist) {
                    if (dbSong.getFilePath() != null) {
                        ...
                        if (file.exists()) {
                            ...
                            uiSong.setPlaylist(true);
                            DATA.add(uiSong);
                        }
                    }
                }
                System.out.println("✅ Đã load " + DATA.size() + " bài hát playlist từ database");
            });
            
        } catch (Exception e) {
            System.err.println("⚠️ Lỗi load playlist từ database: " + e.getMessage());
        }
    }
    ...
    public PlaylistUI(Song.PlayerController controller) {             // [OLD] truyền controller vào UI
        ...
        // ⭐ Load playlist từ database khi khởi động
        loadFromDatabase();
    ...
    private Region makeRow(int index, Song song) {                    // [OLD] xây UI cho 1 bài
        ...
        play.setOnAction(e -> { Utils.PlaybackQueue.setQueue(PlaylistUI.items()); controller.play(song); }); // set queue → play
        ...
        row.setOnAction(e -> { Utils.PlaybackQueue.setQueue(PlaylistUI.items()); controller.play(song); });  // set queue → play
